...
def build_file_lists(images_dir, masks_dir):
    ...
    for sub_folder in sorted([f for f in os.listdir(images_dir) if os.path.isdir(os.path.join(images_dir, f))], key=lambda x: int(x)):
        sub_path = os.path.join(images_dir, sub_folder)
        for f in os.listdir(sub_path):
            if f.lower().endswith(".jpg"):
                images_ls.append(os.path.join(sub_path, f))
                
    for sub_folder in sorted([f for f in os.listdir(masks_dir) if os.path.isdir(os.path.join(masks_dir, f))], key=lambda x: int(x)):
